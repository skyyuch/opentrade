...
進度：~96%（code done, contracts deployed, UI polish done, broker-name i18n 三列 done, translation deprecated, outbox SBT idempotent, review submit + on-chain + IPFS partial E2E verified；剩 c (v) zh-Hans broker page browser check 30 秒 + b L2 verify-broker 完整新-wallet E2E）
...
- [x] **`c` multi-broker E2E 部分驗證** — (i) tier 不升 L2→L3 + (ii) 不重發 mint event：DB query + worker log 確認；(iii) `/verify` 顯示 broker list：user 截圖確認；(iv) ReviewCard 顯示對應徽章：本 session a2 review card UI 已顯示 broker badges。剩 (v) zh-Hans broker page browser check 30 秒待 user 完成
...
- **`c (v)` zh-Hans broker name browser 30 秒驗證**：開 `http://localhost:3000/zh-Hans/brokers/bright-smart-futures-and-commodities-company-limited` 確認 broker 主標題顯示「耀才期货及商品有限公司」（简体）而非「耀才期貨及商品有限公司」（繁体）
...
9. **`c (v)` zh-Hans broker name browser 驗證**（30 秒）— `http://localhost:3000/zh-Hans/brokers/<slug>` 確認 broker 主標題顯示簡體
