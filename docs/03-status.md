...
進度：100%（Commit #1 ~ number-ten 全部 ship；Phase 0 真正收尾，下個 session 進 Phase 1 MVP-A 鏈上評論）

---

## 已完成

### Commit number-ten：CI/CD GitHub Actions（本 session 完成）
...
無（Commit number-ten 全 9 atomic commits 已 ship 並 push；PR #1 全 CI 綠；剩 squash merge + branch protection UI 設定為人工 follow-up）。
...
1. **PR #1 squash merge** 到 `main`（gh pr merge --squash --delete-branch；待本 status commit 後 CI 再跑一輪確認 + merge）
2. **GitHub Settings UI 設定 branch protection rule** for `main`（per ADR-0018 D10 表格）：require status checks（`ci / lint`、`ci / typecheck`、`ci / format`、`ci / test`；conditional `contracts / forge`、`terraform / validate`）+ require PR before merge + require linear history（=squash merge default per rule 70）+ restrict force push + restrict deletions + dismiss stale reviews + require conversation resolution。0 approving reviews（solo Phase 0；2nd contributor 時翻 1）

### 短期（進 Phase 1 MVP-A 前的 housekeeping，可不阻塞 Phase 1）

- 評估 Next.js 14 → 15/16 升級（per docs/03-status.md 開放問題；Dependabot 已 ignore 大版直到 ADR 落地）
- 評估 Prisma 6 → 7 升級時機（per ADR-0013；Dependabot 已 ignore 大版）

### 中期（Phase 1 MVP-A：鏈上評論）

進入 Phase 1。第一步通常是：

1. **Privy 整合**（per ADR-0005 帳號抽象化 + AGENTS.md tech table）
2. **L1/L2 身份系統 schema**（packages/db schema 加 SbtMint、UserIdentity 等表）
3. **ReviewRegistry.sol**（第一個 business contract；同 PR 把 solhint 從 warning-only 切 error-level + extend `solhint:recommended` per ADR-0015 D5；加 Slither 到 contracts.yml）
4. **apps/api `reviews` domain**（per ADR-0006 DDD 四層樣板，照 health domain 模式）
5. **apps/web 寫評論流程**（用 Privy + wagmi v2 + viem 發 tx 上鏈，IPFS 存原文）
...
| 2026-05-21 | Commit number-ten：CI/CD GitHub Actions                        | Claude Opus 4.7 | ADR-0018 CI/CD GitHub Actions architecture（11 個 coordinated decisions + 7 個 alternatives）+ 三 workflow file（`ci.yml` matrix lint/typecheck/test/format + `contracts.yml` forge 1.7.1 + solhint warning-only + `terraform.yml` fmt + per-workspace `init -backend=false` + validate；**完全沒 AWS 認證、完全沒 plan、完全沒 apply**）+ `.terraform.lock.hcl` 從 .gitignore 解禁 + multi-platform hash（linux_amd64/darwin_arm64/darwin_amd64）+ rule 81 加 lock file 章節 + 紅線兩條 + ADR-0018 cross-ref + commitlint scope `conversations` + ESLint `@typescript-eslint/no-restricted-imports` for `apps/{web,console}` 限 `@opentrade/db` + `@prisma/client` runtime import（`import type` allowed per rule 10；用 smoke fixture 驗證後刪）+ Dependabot weekly npm + github-actions ignore Prisma 7+/Next 15+/React 19/Storybook 9 大版 + group `@types/*`/`eslint*`/`@storybook/*`/`next*`/`hono`/`pino`+ CODEOWNERS（Phase 0 solo @skyyuch，granular path 為 onboarding 預留）+ PR template per rule 70 + GitHub CLI 2.92.0 從 release tarball 裝 ~/.local/bin/ + PR #1 開好第一輪 CI 9 個 check 全綠 wall clock < 1 min；剩 squash merge + branch protection UI（ADR-0018 D10）為 follow-up。Phase 0 真正 100% 收尾。 |
