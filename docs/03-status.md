...
### Phase 1: ADR-0026 zh-Hans broker name strategy 實作（本 session 完成）
...
5. ✅ ~~**依 [ADR-0026](./decisions/0026-zh-hans-broker-name.md) 實作 zh-Hans broker name**~~ → 本 session 完成 6 個 commit（DB schema + helper extend + OpenCC backfill + sync-brokers 自動轉 + API 10 endpoints + web/console types + rule 51 升 ship 三列）。Production DB backfill 待 ADR-0026 implementation notes step 6 安排，dev DB 已全 3482 broker 落值
6. **Console Google UI 修復** — globals.css 主題系統與 Google 硬編碼色的衝突需要系統性解決（可能需要 console 專屬 Tailwind preset 或完全繞過共用主題）
7. **端到端瀏覽器測試** — Privy login → exchange JWT → browse brokers → submit review → verify broker (L2 上傳檔案) → admin approve → SBT mint → outbox worker → on-chain confirm。**新增測試點**：同一 user 通過 broker A 後通過 broker B，確認 (a) approve 不升 L2→L3、(b) 不重發 mint event、(c) /verify 顯示 broker list、(d) 評論卡顯示對應徽章 (e) 切到 zh-Hans 確認 broker 名稱顯示簡體（非繁中 fallback）
8. **Outbox worker 本地跑通** — 確認 review 提交後 worker 能 poll → submit on-chain → update status CONFIRMED
9. **Production DB zh-Hans backfill** — 等 dev 驗證 OK 後在 prod RDS 跑 `db:backfill:zh-hans`（per ADR-0026 implementation notes step 6）
