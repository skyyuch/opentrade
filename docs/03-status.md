...
**Phase 1 MVP-A：鏈上評論 — IN PROGRESS**

進度：~40%（Block 1-3 / 7 完成；Block 4 待開始。高階計畫見 `.cursor/plans/phase_1_mvp-a_plan_94edcdde.plan.md`）

---

## 已完成

### Phase 1 MVP-A Block 1-3（本 session 完成）

Branch: `feature/phase-1-mvp-a`（4 commits，已 push）
...
- **Phase 1 MVP-A Block 4**: reviews domain API（Review Prisma model + IPFS Pinata integration + DDD four-layer + `POST /v1/reviews` + outbox → chain tx）
- **Phase 1 MVP-A Block 5-7**: 待 Block 4 完成後依序推進（Block 5: reviews UI + i18n、Block 6: merchant console basics、Block 7: SFC broker seed data）
...
1. **Block 4: reviews domain API** — Prisma `Review` model + migration + IPFS Pinata 整合 + DDD 四層（domain/application/infrastructure/presentation）+ `POST /v1/reviews`（auth middleware gated）+ outbox event for chain tx
2. **Block 5: reviews UI** — apps/web broker 頁面 + 寫評論流程（Privy + wagmi v2 + viem 發 tx 上鏈）+ i18n
3. **Block 6: merchant console basics** — Privy 登入 + claim broker page + 看評論
4. **Block 7: SFC broker seed data** — 爬取 + 匯入腳本

### 短期（housekeeping，不阻塞 Phase 1）

- PR #1（Phase 0 CI/CD）已 squash merge + branch protection 已設定完成
...
- ✅ ~~**API 認證流程**：`JWT_SECRET` 目前是 placeholder~~ → 已實作 ES256 JWT（`JWT_PRIVATE_KEY_PEM` + `JWT_PUBLIC_KEY_PEM`）+ Privy token exchange `POST /v1/auth/exchange` + auth middleware with role hierarchy（Block 2c+2d）
