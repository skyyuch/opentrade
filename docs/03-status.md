...
- **ADR-0026 zh-Hans broker name 實作**（next session）：方案已拍板（DB 加 `displayNameZhHans String?` + OpenCC 一次性 backfill + sync-brokers 自動轉），6-commit 計畫待 fresh session 動工。詳見 [ADR-0026](./decisions/0026-zh-hans-broker-name.md) Implementation Notes 段。
...
5. **依 [ADR-0026](./decisions/0026-zh-hans-broker-name.md) 實作 zh-Hans broker name**（next session 主任務）：6-commit 計畫，預計 1 session 完成
   - `feat(db)` add `displayNameZhHans String?` column + migration
   - `feat(shared)` extend `localizedBrokerName` helper for `zh-Hans`
   - `feat(db)` OpenCC backfill script + `sync-brokers.ts` integration
   - `feat(api)` ship displayNameZhHans on 9 endpoints
   - `feat(web,console)` types + verify all consumers
   - `docs(rules,status)` rule 51 升級 ship 三列 + status update
6. **Console Google UI 修復** — globals.css 主題系統與 Google 硬編碼色的衝突需要系統性解決（可能需要 console 專屬 Tailwind preset 或完全繞過共用主題）
7. **端到端瀏覽器測試** — Privy login → exchange JWT → browse brokers → submit review → verify broker (L2 上傳檔案) → admin approve → SBT mint → outbox worker → on-chain confirm。**新增測試點**：同一 user 通過 broker A 後通過 broker B，確認 (a) approve 不升 L2→L3、(b) 不重發 mint event、(c) /verify 顯示 broker list、(d) 評論卡顯示對應徽章
