...
3. ✅ ~~**依 ADR-0025 補多券商實作**~~ → 本 session 已完成 6 個 commit（DB schema → API approve flow → API response → /verify UI → console UI → review badge）。Phase 2 觸發點仍待 — 當（a）有用戶被驗證 ≥ 3 家 broker，或（b）KOL/jury 上鏈需要 broker mapping 時，才升 ReviewerSBT v2
4. **Console Google UI 修復** — globals.css 主題系統與 Google 硬編碼色的衝突需要系統性解決（可能需要 console 專屬 Tailwind preset 或完全繞過共用主題）
5. **端到端瀏覽器測試** — Privy login → exchange JWT → browse brokers → submit review → verify broker (L2 上傳檔案) → admin approve → SBT mint → outbox worker → on-chain confirm。**新增測試點**：同一 user 通過 broker A 後通過 broker B，確認 (a) approve 不升 L2→L3、(b) 不重發 mint event、(c) /verify 顯示 broker list、(d) 評論卡顯示對應徽章
