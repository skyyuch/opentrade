...
- **日期**：2026-05-24
- **更新者**：Console admin nav bug fix session（Claude Opus 4.7）
- **本次更新摘要**：修復 ADR-0024 已知 bug — admin menu 點擊跳回總覽 / 404 / webpack `originalFactory.call` 錯誤。三個根因：(1) console 用 `localePrefix: 'as-needed'`，App Router 把 `/admin/users` 誤匹配為 `[locale=admin]/users` 而 404 → 改用 `'always'`（console 不索引，as-needed SEO 優勢不適用）；(2) `AuthGate` sidebar 用 `next/link` + 硬編碼 `/${locale}` 前綴與 next-intl 衝突 → 改用 `i18n/navigation` 的 locale-aware `Link`；(3) `UsersClient` 在 `.map()` 用無 key 的 Fragment 導致 webpack HMR factory cache 損壞 → 改用 `Fragment key={u.id}`。另強化 AdminGuard/BrokerGuard：將 `router.replace()` 移到 `useEffect`（避免 render-time side effect），並改用 next-intl 的 `useRouter`。
