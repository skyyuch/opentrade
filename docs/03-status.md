...
- **日期**：2026-05-21
- **更新者**：Commit number-nine session（Claude Opus 4.7）— Terraform IaC 上線 + apps/api Dockerfile + Phase 0 第 9 個 commit ship
- **本次更新摘要**：Commit number-nine 真實 ship — Terraform IaC 雛形（`infra/terraform/{bootstrap,environments/dev,modules/{vpc,rds-postgres,ecs-fargate-cluster,ecr-repo,frontend-cdn,secrets}}`，6 個 module + 1 bootstrap + 1 environment）+ apps/api 多階段 Dockerfile（Debian slim + tsup bundle + pnpm deploy --ignore-scripts + Prisma engine 從 .pnpm store 注入，image 554 MB）+ ADR-0017（11 個決策含 module 4-file convention、Phase-0 apply-not-plan-only 改寫、~$54/mo cost envelope、identity guard、default tags、apply 紅線）+ 新 cursor rule `81-terraform-iac.mdc`（`alwaysApply: true`，codify ADR-0017 為 operational 紀律：4-file 紀律、cost-tuning toggle 慣例、secret-never-in-state、apply-only-against-dev、SCP/dual-NAT/Multi-AZ Phase 4+ deferred）+ 真實 `terraform apply` 對 `opentrade-dev`：56 resources 建好（VPC `vpc-07de0826512fd588b`、2× public subnet、2× private subnet、IGW、1× NAT、3× route table、VPC flow logs、RDS Postgres 16.14 `opentrade-dev-postgres.c12wwm68i3oo.ap-southeast-1.rds.amazonaws.com:5432`、RDS-managed master password secret、ECS cluster `opentrade-dev-cluster`、ECS task-exec role + task role + log group、ECR repo `371637912734.dkr.ecr.ap-southeast-1.amazonaws.com/opentrade-api`、2× CloudFront 分送 web `https://d2vx070o8286j9.cloudfront.net` + console `https://d1b00mlhv5lfyy.cloudfront.net`（後者帶 `X-Robots-Tag: noindex, nofollow` per ADR-0010）、3× Secrets Manager 空 slot `opentrade/dev/{jwt-secret,privy-app-secret,deepl-api-key}`）+ Docker push 端到端驗證（`opentrade-api:dev` tag 推到 ECR，digest `sha256:d2691b347e77eb0cb2e90819ec5659d47b504588e0b5b1af7a03d7217686c85b`）+ Terraform v1.15.4 安裝到 `~/.local/bin/`（`~/.zshrc` 補 PATH）+ `docs/02-roadmap.md` Phase-0 DoD 從 "plan only" 改為 "apply against opentrade-dev cleanly"。Apply 過程中遇 RDS engine version 16.4 在 ap-southeast-1 不可用，bump default 到 16.14（per `aws rds describe-db-engine-versions` 查詢結果）後 second apply 收尾。Phase 0 進度 100%，下一個 commit 是 number-ten CI/CD GitHub Actions。
...
進度：100%（Commit #1 ~ number-nine 全部 ship；下一階段是 Phase 0 收尾的 Commit number-ten CI/CD）

---

## 已完成

### Commit number-nine：infra/terraform 雛形 + apps/api Dockerfile（本 session 完成）
...
無（Commit number-nine 全部完成 + commit + push 待跑；下個 session 開 Commit number-ten CI/CD）。
...
1. **Commit number-ten：CI/CD GitHub Actions** — lint + typecheck + test + migrate 在 PR 上自動跑；Renovate / dependabot 排除 Prisma 7.x（per ADR-0013）；加 ESLint 規則阻止前端 runtime import `@opentrade/db`（per Commit #4 conversation 提到的紀律強化）；console 的 `X-Robots-Tag: noindex, nofollow` edge header 已在 CloudFront response-headers policy 設好（per ADR-0010 + Commit number-nine `frontend-cdn` module），CI 不需重複；**`foundry-toolchain` action 在 contracts job 設好**（per ADR-0015 implementation notes，pin forge 1.7.x + `git submodule update --init --recursive`）；solhint 在 PR 跑時仍 warning-only（per ADR-0015 D5）但 `forge build / forge test / forge fmt --check` 為 hard gate；**Terraform 也要進 CI**（`terraform fmt -check -recursive` + `terraform validate` per env，per ADR-0017 + rule 81 不可在 CI `apply`）；**首次 Terraform apply 後**（已完成 Commit number-nine）才能加 GitHub OIDC 把 ECR push 自動化（Phase 1 起，per ADR-0017 D11）
