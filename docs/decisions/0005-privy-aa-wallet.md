...
Accepted (Amended 2026-05-27 — drop SMS, add iAM Smart login per ADR-0031 D7)
...
1. 使用者用「Web 2.0 體驗」登入：Google、Apple、Email
2. 進階用戶可用自管錢包（Wallet Connect）或香港智方便（iAM Smart）登入
3. 系統自動為他們生成錢包（Account Abstraction）
4. 使用者操作鏈上動作時 gas 由平台代付（Gasless）
5. 但**錢包完全屬於使用者**（去中心化精神不可違背）
...
1. 使用者點擊「登入」
2. 看到 Google / Apple / Email / Wallet / 智方便（iAM Smart）五種選項
...
1. 前端 redirect 到 OGCIO 授權 URL（或顯示 QR code 供手機掃碼）
2. 使用者在智方便 app 確認身份
3. OGCIO 回撥 OpenTrade callback URL
4. API 驗證身份 + 計算 `identityHash = SHA-256(HKID + salt)` + 建立/查找 user
5. 同步完成 Privy embedded wallet provision + 身份驗證（L1 + identity verification 一步到位）
6. 簽發 OpenTrade JWT

詳見 [ADR-0031 D7](./0031-iam-smart-identity-provider.md)。
...
// apps/web/src/providers/privy.tsx (僅示意)
{
  ...
  // iAM Smart 不是 Privy native method，走自訂 OAuth 整合
  // 見 ADR-0031 D7 iAM Smart login endpoint
  embeddedWallets: {
...
> **2026-05-27 變更**：移除 `'sms'`（SMS 認證不再支援，因香港預付 SIM 卡便宜，Sybil 防範力低）。iAM Smart 登入走獨立 OAuth 路徑而非 Privy native method，因為 Privy 不原生支援政府 OIDC provider。
...
- ❌ 把 iAM Smart 的 HKID / 真名寫進 log 或 DB 明文（只存 hash，見 ADR-0031 D4）

## References

- [Privy 官方文件](https://docs.privy.io/)
- [ERC-4337 規範](https://eips.ethereum.org/EIPS/eip-4337)
- [ADR-0031: iAM Smart Identity Provider](./0031-iam-smart-identity-provider.md) — iAM Smart 登入 + 身份驗證雙角色
