...
| S2      | ADR-0031     | iAM Smart (智方便) integration for L3 jury identity                 | Phase 3 jury infrastructure work begins OR HKMA Sandbox pilot requires it           | Coexists with [ADR-0005](./0005-privy-aa-wallet.md); does not supersede                               | Medium (Phase 3 blocker)                 |
...
| S4      | ADR-0033     | Public broker response mechanism (Porto 聲明窗口)                   | M10 商戶功能 Phase 2.5 implementation kickoff                                       | Folded into ADR-0037 if M10 ships first                                                               | Low (deferred until M10)                 |
...
| S7      | ADR-0036     | KOL signal architecture (Phase 2)                                   | M8 KOL ADR drafting kickoff                                                         | New domain; sits next to [ADR-0019](./0019-review-registry-contract-design.md) ReviewRegistry pattern | High (Phase 2 critical-path)             |
| S8      | ADR-0037     | Merchant editable scope + ad isolation (Phase 2.5)                  | M10 商戶功能 Phase 2.5 implementation kickoff                                       | Defines C5 (S4 absorbs in if M10 ships first)                                                         | High (Phase 2.5 critical-path)           |
...
2. **Blocked on infrastructure not yet built**. S2 (iAM Smart) is blocked on Phase 3 jury system existing; the integration shape depends on how L3 identity is checked.
3. **Blocked on external timing**. S3 (HKMA Sandbox) is blocked on the grant application being formally filed.
4. **Implementation-bundled**. S4 (broker response) and S8 (merchant scope) are most coherent as one ADR if M10 ships them together; we will collapse S4 into S8's body unless M10 splits them.
...
7. **Implementation-imminent**. S7 (KOL) and S8 (Merchant) are the next two implementation milestones; their ADRs land in M8 and M10 respectively, ahead of any code.
