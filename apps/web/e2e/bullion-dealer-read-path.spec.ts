/**
 * ADR-0045 §6: bullion-dealer (HKGX) list → detail → tab-switch read-path
 ...
 * `hkgx-009` detail, its reviews, and an empty complaint list. The flow
 ...
 * the HKGX membership header pill, and the bullion-only tab variant.
 */
...
test.describe('Bullion dealer — read-path', () => {
  test('lists HKGX members on the bullion directory', async ({ page }) => {
    await page.goto('/en/bullion-dealers');

    await expect(page.getByRole('heading', { name: /Bullion Dealers/ })).toBeVisible();
    // The card renders the HKGX 行員 number as its headline trust badge and
    // links to the namespaced bullion route (never the securities /brokers/).
    await expect(page.getByText('HKGX Member 009')).toBeVisible();
  ...
  test('navigates to the dealer detail and shows the HKGX membership header', async ({ page }) => {
    await page.goto('/en/bullion-dealers');
    ...
    // The header carries the HKGX membership pill (number from the license).
    await expect(page.getByText(`HKGX Member ${SEED.bullionMemberNumber}`)).toBeVisible();
  });
  ...
  test('renders the bullion-only tab set (no SFC licence tab)', async ({ page }) => {
    ...
    // HKGX carries no regulated-activity detail, so the SFC licence tab,
  ...
  test('switches between Membership and Complaints tabs', async ({ page }) => {
    ...
    await expect(page.getByText('HKGX Membership Record')).toBeVisible();
