 ...
 * carry all three name columns — `displayName` (Traditional Chinese),
 * `displayNameZhHans` (Simplified Chinese, OpenCC-derived per ADR-0026),
 * and `legalName` (English) — so the frontend can pick the right column
 * with `localizedBrokerName(b, locale)` from `@opentrade/shared`. Sending
 * only the slug forces consumers to render
 * `hsbc-broking-securities-hong-kong-limited` (the actual user-reported
 * regression on /verify and /admin/verifications); sending only
 * `displayName` (the previous shortcut) leaks Chinese into English-locale
 * UIs; omitting `displayNameZhHans` leaks Traditional Chinese into
 * Simplified-locale UIs.
 *
 ...
 *
 * `displayNameZhHans` is null when the OpenCC backfill rejected the row
 * (rare; logged in dev as the "skipped" counter) — the helper treats
 * null as a fallback signal so consumers don't need to special-case it.
 */
export type BrokerNameMeta = {
  readonly displayName: string;
  readonly displayNameZhHans: string | null;
  readonly legalName: string | null;
};

/**
 * Fetch `{ displayName, displayNameZhHans, legalName }` for every
 * requested slug in a single indexed query, returning a `Map` keyed by
 * slug. Slugs that don't exist (e.g. dangling `user_verified_brokers`
 * rows after a broker is soft-deleted) are simply absent from the map;
 * callers should fall back to the slug itself in that case.
 *
 ...
 * `displayName + displayNameZhHans + legalName` triple pulled from the
 * hydration map. Rows whose slug is missing from the map fall back to
 * `{ displayName: slug, displayNameZhHans: null, legalName: null }` so
 * the consumer never gets `undefined`.
 */
