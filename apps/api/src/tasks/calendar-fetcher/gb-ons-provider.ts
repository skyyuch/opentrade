 ...
 * batch 2).
 *
 * The ONS is the UK's official statistical authority. Its public release
 * calendar is exposed as a key-less JSON API
 * (`api.beta.ons.gov.uk/v1/search/releases`) that lists each dated release with
 * a stable `/releases/<slug><period>` uri, a title, an ISO-UTC `release_date`,
 * and `published` / `cancelled` flags. This provider fetches the upcoming and
 * recently-published releases and maps each to a configured indicator by the
 * stable slug prefix (`onsUriPrefix`) — never a fuzzy title search — excluding
 * the `…timeseries` companion release that shares a date/period.
 *
 * Compliance (ADR-0058 D1): the ONS releases API exposes the schedule only, not
 ...
 * the authority's official bulletin via the config registry (`sourceUrl`).
 *
 * Per-event failures are isolated: one malformed entry can never block the
 * others (mirrors the Eurostat / FRED providers).
 */
 ...
 * `…timeseries` duplicate, or falls outside the window. Values are always null
 * — the ONS releases calendar carries no figures.
 */
function toDraft(
  entry: OnsRawRelease,
